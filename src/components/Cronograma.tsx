...
import { Calendar, Layers, Clock, Shield, Sparkles, RefreshCw, BadgeHelp, CheckCircle2, AlertCircle, Sliders, Check, SlidersHorizontal, ArrowRight } from 'lucide-react';
...
export default function Cronograma({ schedule, onboardingData, onUpdateSchedule }: CronogramaProps) {
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-8" id="cronograma-view-container">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-slate-800">
        ...
        <div className="flex items-center gap-3 self-start sm:self-auto">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-xs font-bold text-yellow-500 hover:text-yellow-400 hover:border-yellow-500/25 hover:bg-slate-905 transition-all disabled:opacity-45"
      ...
      {/* 2. CONFIGURADOR DE CARGA HORÁRIA INTELIGENTE (Weekly & Daily Hours customizer) */}
      {!isRecalibrating && (
        <div className="bg-slate-950/80 border border-slate-850 p-5 sm:p-6 rounded-xl space-y-6 text-left" id="hours-configurator-card">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-4 border-b border-white/[0.04]">
            <div className="flex items-start gap-3">
              <div className="p-2.5 bg-amber-500/10 rounded-lg text-amber-500 border border-amber-500/20 mt-1 shrink-0">
                <Sliders className="w-5 h-5" />
              </div>
              <div className="space-y-1">
                <h3 className="text-sm sm:text-base font-bold text-white flex items-center gap-2">
                  <span>Ajustes Personalizados de Carga Horária</span>
                  <span className="text-[10px] bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 font-mono px-2 py-0.5 rounded uppercase font-bold">Ativo</span>
                </h3>
                <p className="text-xs text-slate-400 leading-relaxed max-w-xl">
                  Defina a carga horária de sua rotina por semana ou declare o tempo que você pode se dedicar para cada dia. O sistema distribuirá instantaneamente a duração de cada matéria de forma matemática.
                </p>
              </div>
            </div>

            {/* Range controls total */}
            <div className="bg-slate-900 border border-slate-850 p-3 rounded-xl min-w-[220px] shrink-0">
              <div className="flex justify-between items-center text-[10px] font-mono text-slate-400 tracking-wider uppercase font-bold text-slate-500">
                <span>META SEMANAL</span>
                <span className="text-amber-500 text-[11px]">CALCULADO</span>
              </div>
              <span className="text-lg font-mono font-black text-white block mt-1">
                {totalWeeksHours} <span className="text-xs text-slate-400 font-normal">horas / semana</span>
              </span>
          ...
          {/* Daily hours row controls */}
          <div className="space-y-3">
            <span className="text-xs font-mono uppercase text-slate-400 font-bold tracking-wider block">Insira as horas que você pode estudar em cada dia:</span>
            <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-7 gap-3">
              {['Segunda', 'Terça', 'Quarta', 'Quinta', 'Sexta', 'Sábado', 'Domingo'].map((cleanDay) => {
                ...
                return (
                  <div 
                    key={cleanDay} 
                    className={`bg-slate-900 border p-3 rounded-xl text-center flex flex-col justify-between space-y-2.5 transition-all relative ${
                      hours === 0 ? 'border-dashed border-emerald-950 bg-emerald-955/5' : 'border-slate-850'
                    }`}
                  >
                    ...
                    <div className="font-mono text-base font-black text-white">
                      {hours === 0 ? (
                        <span className="text-[10px] px-1.5 py-0.5 bg-emerald-950/40 text-emerald-400 font-bold tracking-wider uppercase rounded">Folga</span>
                      ) : (
                        `${hours}h`
                      )}
                    </div>
                    ...
                    <div className="flex items-center justify-center gap-1">
                      <button
                        ...
                        className="w-7 h-7 bg-slate-950 hover:bg-slate-800 text-slate-300 hover:text-white border border-slate-850 rounded-lg flex items-center justify-center text-xs font-bold transition-all active:scale-95 cursor-pointer"
                      >
                        -
                      </button>
                      <button
                        type="button"
                        onClick={() => handleUpdateHours(cleanDay, hours + 0.5)}
                        className="w-7 h-7 bg-slate-950 hover:bg-amber-500 border border-slate-850 rounded-lg flex items-center justify-center text-xs font-bold text-amber-500 hover:text-slate-950 transition-all active:scale-95 cursor-pointer"
                      >
                        +
                      </button>
                    </div>
      ...
      {selectedSlot && (
        <div className="fixed inset-0 bg-black/75 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-amber-500/30 max-w-md w-full rounded-2xl p-6 shadow-2xl relative animate-scale-up text-left">
            <div className="flex items-start gap-4 mb-4">
              <div className="p-3 bg-amber-500/10 rounded-xl border border-amber-500/20 text-amber-400 text-xl font-bold">
                🦉
              </div>
              <div className="min-w-0">
                <dt className="text-xs uppercase font-mono tracking-widest text-amber-400 font-semibold">{selectedSlot.day}</dt>
                <h3 className="text-base sm:text-lg font-bold text-white leading-tight mt-0.5 truncate">{selectedSlot.discipline}</h3>
                <span className="text-xs text-slate-400 font-mono italic block truncate">Tópico: {selectedSlot.topic}</span>
              </div>
            </div>

            <div className="bg-slate-950/90 p-4 border border-slate-850 rounded-xl mb-6 text-xs sm:text-sm text-slate-205 leading-relaxed italic text-slate-300">
            ...
            <div className="flex items-center justify-between text-xs text-slate-500 font-mono mb-4 border-b border-slate-900 pb-4">
              <span className="flex items-center gap-1 shrink-0"><Clock className="w-3.5 h-3.5" /> Duração: {selectedSlot.duration} minutos</span>
              <span className="bg-slate-950 px-2.5 py-0.5 rounded text-amber-400 uppercase text-[9.5px] border border-white/[0.04]">{selectedSlot.activityType}</span>
            </div>
