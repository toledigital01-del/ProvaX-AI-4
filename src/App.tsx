...
import { 
  ...
  CreditCard, Sparkles, BookMarked, Menu, X, Sun, Moon, Volume2, VolumeX, Shield
} from 'lucide-react';
...
export default function App() {
  ...
  const renderActiveTab = () => {
    ...
    switch (currentTab) {
      ...
      case 'progresso':
        return <Progresso progress={progress} />;
      case 'biblioteca':
        return (
          <Biblioteca 
            library={library}
  ...
  if (!onboarding) {
    ...
    return (
      <div className={`min-h-screen bg-slate-950 text-slate-150 flex flex-col font-sans select-none relative ${theme === 'light' ? 'light-theme prf-theme' : ''}`} id="provax-application-workspace">
  ...
  return (
    <div className={`min-h-screen bg-slate-950 text-slate-150 flex flex-col font-sans select-none ${theme === 'light' ? 'light-theme prf-theme' : ''}`} id="provax-application-workspace">
      ...
      <header className="bg-slate-900 border-b border-emerald-950/40 p-4 sticky top-0 z-40 flex items-center justify-between shadow-md" id="control-panel-header">
        ...
        <div className="flex items-center gap-3">
          ...
          {/* Sound Mute/Unmute Switcher */}
          <button 
            onClick={toggleSound}
            className="p-2 hover:bg-slate-850 border border-slate-850 bg-slate-950 rounded-lg text-slate-400 hover:text-white cursor-pointer transition-colors flex items-center justify-center hidden sm:flex"
            title={soundOn ? 'Desativar Efeitos Sonoros' : 'Ativar Efeitos Sonoros'}
            id="sound-switcher-header-btn"
          >
            {soundOn ? (
      ...
      <div className="flex-1 flex max-w-7xl w-full mx-auto" id="application-body-layout">
            ...
            <div className="w-64 h-full bg-slate-950 border-r border-slate-900 p-5 space-y-6 flex flex-col justify-between animate-slide-right">
              ...
              <div className="border-t border-slate-900 pt-4 space-y-3">
                ...
                <div className="flex items-center justify-around gap-2">
                  ...
                  <button 
                    onClick={toggleTheme}
                    className="p-2.5 flex-1 hover:bg-slate-850 border border-slate-850 bg-slate-900 rounded-lg text-slate-400 hover:text-white transition-colors flex justify-center items-center gap-1.5 text-xs font-mono font-bold"
                    title={theme === 'dark' ? 'Ativar Modo Claro' : 'Ativar Modo Escuro'}
                  >
                  ...
                  {/* Sound Switcher inside drawer */}
                  <button 
                    onClick={toggleSound}
                    className="p-2.5 flex-1 hover:bg-slate-850 border border-slate-850 bg-slate-900 rounded-lg text-slate-400 hover:text-white transition-colors flex justify-center items-center gap-1.5 text-xs font-mono font-bold"
                    title={soundOn ? 'Desativar Efeitos Sonoros' : 'Ativar Efeitos Sonoros'}
                  >
                    {soundOn ? <Volume2 className="w-4 h-4 text-emerald-400" /> : <VolumeX className="w-4 h-4 text-red-500" />}
                    <span>{soundOn ? 'Sim' : 'Não'}</span>
                  </button>
                </div>
